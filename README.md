...
*If markdown can write formated text, why not HRMIDI can write music ?*

# The language:
In the top of each music, must be declared some global details:
```
BPM 100 // The speed of the music
LANG en // The human language used to call the instruments (not all supported at the moment)
```
The instruments can be declared like that:
```
C_PIANO bass				# Decides the clef of this section
...
M_VIOLIN 2				# Modificator, here means twice faster as others
```
The language have some rules:
- Comments can be writen using `#` or `//` at the ending of the line;
- The terms/keys must be split with *space* ` `, *tabs* `	` and/or semicolons `;`. The number used between two different keys dont matter, this is for formating reasons;
- Lines must be split using `\n` (new line);
- Empty lines will be ignored, you can use them for formating.
- Keys/terms can be writen in lower or higher case, and even mix both.
